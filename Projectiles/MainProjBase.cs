...
namespace AntiverseMod.Projectiles {
	public abstract class MainProjBase : ModProjectile {
		...
		public sealed override void OnHitNPC(NPC target, int damage, float knockback, bool crit) {
			OnHit(new EntityHelper.EntityRef(target), damage, knockback, crit);
		}

		public sealed override void OnHitPlayer(Player target, int damage, bool crit) {
			OnHit(new EntityHelper.EntityRef(target), damage, null, crit);
		}

		public sealed override void OnHitPvp(Player target, int damage, bool crit) {
			OnHit(new EntityHelper.EntityRef(target), damage, null, crit, true);
		}

		/// <summary>
		/// Allows you to make special effects when this projectile hits an entity. Pvp is true when the player hit is an opponent
		/// </summary>
		public virtual void OnHit(EntityHelper.EntityRef target, int damage, float? knockback, bool crit, bool pvp = false) {
		}
		...
		public sealed override void ModifyHitNPC(NPC target, ref int damage, ref float knockback, ref bool crit, ref int hitDirection) {
			ModifyHit(new EntityHelper.EntityRef(target), ref damage, ref knockback, ref crit, ref hitDirection);
		}
		...
		public sealed override void ModifyHitPvp(Player target, ref int damage, ref bool crit) {
			float knockback = 0;
			int hitDirection = 0;
			ModifyHit(new EntityHelper.EntityRef(target), ref damage, ref knockback, ref crit, ref hitDirection, true);
		}
		...
		/// </summary>
		public virtual void ModifyHit(EntityHelper.EntityRef target, ref int damage, ref float knockback, ref bool crit, ref int hitDirection, bool pvp = false) {
		}
	}
}
